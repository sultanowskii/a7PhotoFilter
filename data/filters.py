...
def add_filter(image_object, fid):
    ...
    x, y = im.size
    fid = int(fid)
    if fid == 1:  # прикольная шняга 1 (1)
        color_inversion1(pixels, x, y)
        return im
    elif fid == 2:  # прикольная шняга 2 (2)
        color_inversion2(pixels, x, y)
        return im
    elif fid == 3:  # Чёрно-белое (3)
        black_white(pixels, x, y)
        return im
    elif fid == 4:  # негатив (4)
        inversion(pixels, x, y)
        return im
    elif fid == 5:  # высветление (5)
        highlighting(pixels, x, y)
        return im
    elif fid == 6:  # размытие (6)
        return degradation(im)
    elif fid == 7:  # увеличение резкости (7)
        return sharpness(im)
    elif fid == 8:  # уменьшение качества (8)
        reducing_the_quality(pixels, x, y)
        return im
    elif fid == 9:  # добавление шумов(9)
        add_noise(pixels, x, y)
        return im
    elif fid == 10:  # как в 90-е(10)
        brown(pixels, x, y)
        return im
    elif fid == 11:  # чисто чёрно-белоя картинка(11)
...
def color_inversion1(pixels, x, y):  # прикольная шняга 1 (1)
    for i in range(x):
        for j in range(y):
            r, g, b = pixels[i, j]
            pixels[i, j] = g, b, r
...
def color_inversion2(pixels, x, y):  # прикольная шняга 2 (2)
    for i in range(x):
        for j in range(y):
            r, g, b = pixels[i, j]
            pixels[i, j] = b, r, g
...
def add_noise(pixels, x, y):  # добавление шумов(9)
    factor = 70
    for i in range(x):
        for j in range(y):
...
def brown(pixels, x, y):  # как в 90-е(10)
    depth = 30
    for i in range(x):
        for j in range(y):
            r, g, b = pixels[i, j]
...
def all_black_white(pixels, x, y):  # чисто чёрно-белоя картинка(11)
    factor = 100
    for i in range(x):
        for j in range(y):
            r, g, b = pixels[i, j]
